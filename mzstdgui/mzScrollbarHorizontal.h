...
#ifndef __MZSTDGUI_SCROLLBAR_HORIZONTAL_H__
#define __MZSTDGUI_SCROLLBAR_HORIZONTAL_H__
...
class DECLSPEC_MZSTDGUI CScrollbarHorizontal : public CScrollbar {
public:
	WINAPI CScrollbarHorizontal(CSprite *parent, mzstd::CmzString name, LONG x = 0, LONG y = 0, LONG position = 1, LONG maxValue = 100, LONG minValue = 1, BOOL autosize = FALSE);
	virtual WINAPI ~CScrollbarHorizontal();
    ...
    virtual BOOL WINAPI mouseOnArrowLeft(LONG localX, LONG localY);
    virtual BOOL WINAPI mouseOnArrowRight(LONG localX, LONG localY);
    virtual BOOL WINAPI mouseOnBgLeft(LONG localX, LONG localY);
    virtual BOOL WINAPI mouseOnBgRight(LONG localX, LONG localY);

private:
    virtual VOID WINAPI _updateCanvas();
};
typedef CScrollbarHorizontal* PScrollbarHorizontal;
